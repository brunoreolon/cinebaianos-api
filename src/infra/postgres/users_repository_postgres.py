...
class UserRepositoryPostgres(UsersRepository):
    ...
    def register_user(self, discord_id: str, name: str, tab: str, column: str, email: str, password: str) -> User:
        with self.conn_provider.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                           INSERT INTO users (discord_id, name, tab, col, email, password)
                           VALUES (%s, %s, %s, %s, %s, %s)
            ...
            cursor.execute("""
                           SELECT discord_id, name, tab, col, email, password FROM users WHERE discord_id = %s
                           """, (discord_id,))
            row = cursor.fetchone()
            if row:
                return User(
    ...
    def get_user(self, discord_id: str) -> User:
        with self.conn_provider.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                           SELECT discord_id, name, tab, col, email, password
                           FROM users
                           WHERE discord_id = %s
                           """, (discord_id,))
            row = cursor.fetchone()
